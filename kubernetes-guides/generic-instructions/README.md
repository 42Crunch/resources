...
This document describes how to deploy 42Crunch API Firewall as a sidecar container on any Kubernetes cluster. For more information on 42Crunch Platform and API Firewall, see [42Crunch Platform documentation](https://docs.42crunch.com/latest/content/home.htm).
...
1. Validating connection to existing Kubernetes cluster.
...
The 42Crunch API firewall image is located on a private DockerHub repository: you must have been given access to it. If you are an existing platform user but cannot access the Docker image, send a mail to: support@42crunch.com.
...
- `Pixiapp`, which exposes the original, unprotected Pixi API so that you can directly invoke the vulnerable API for testing.

Both deployments are fronted by load balancers and point a Mongo database deployed in a pod called `pixidb`, exposed via a service.

![Demo Architecture](/kubernetes-guides/graphics/GenericDeployment.jpg)
...
6. Copy the protection token value to the clipboard. **Do not close this dialog** until you have safely saved the token value (in the next step).
...
1. Go to edit the file `kubernetes-artifacts/etc/secret-protection-token` located under Kubernetes-artifacts.
...
1. Depending on your platform, run either the `pixi-create-demo.sh` or `pixi-create-demo.bat` script located under Kubernetes-artifacts to deploy the sample configuration:
...
2. Once the deployment is complete, run `kubectl get pods -n 42crunch` to check that all pods are successfully running:
   ...
   ```
### Step 4 - Getting ready to test the API firewall
...
2. Edit your `hosts` file, and add the `pixisecured` deployment to it. Replace the placeholders `<pixi-secu-ip>`and `<pixi-app-ip>` with the actual external IPs returned by the command above:
...
7. The Pixi API has a MongoDB injection vulnerability that allows logging in to the application without specifying a password. You can try this by using the raw parameters `user=42crunch@getme.in&pass[$ne]=` in Postman for a login request. You will see that you can log in to the unprotected API, but the request is blocked by API Firewall on the protected API.
